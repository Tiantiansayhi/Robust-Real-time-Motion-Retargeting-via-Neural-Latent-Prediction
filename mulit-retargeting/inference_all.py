...
if __name__ == '__main__':
    ...
    for key in keys:
        ...
        print(key)
        test_data = parse_all(filename=cfg.INFERENCE.MOTION.SOURCE, selected_key = key, sliding_window = cfg.HYPER.PERIOD)
        ...
        # Create model
        model_r = getattr(model, cfg.MODEL.NAME)(cfg.HYPER.PERIOD).to(device)

        seq_length = cfg.HYPER.PERIOD    

        # Load checkpoint
        if cfg.MODEL.CHECKPOINT is not None:
            model_r.load_state_dict(torch.load(cfg.MODEL.CHECKPOINT))
        ...
        for epoch in range(cfg.HYPER.EPOCHS):
            train_loss = train_epoch(model_r, ee_criterion, vec_criterion, col_criterion, lim_criterion, ori_criterion, fin_criterion, reg_criterion, smo_criterion, optimizer, test_loader, test_target, epoch, seq_length, logger, cfg.OTHERS.LOG_INTERVAL, writer, device, z_all)
        ...
        for batch_idx, data_list in enumerate(test_loader):
            for target_idx, target in enumerate(test_target):
                ...
                z = z_all[batch_idx]
                # forward
                _, target_ang, _, _, target_global_pos, l_hand_ang, l_hand_pos, r_hand_ang, r_hand_pos = model_r.decode(z, Batch.from_data_list(target_list).to(z.device))
                ...
                if l_hand_pos is not None and r_hand_pos is not None:
                    ...
                    decode_end_time = time.time()
        print('total time {} ms'.format((decode_end_time - encode_start_time)*1000)) 
