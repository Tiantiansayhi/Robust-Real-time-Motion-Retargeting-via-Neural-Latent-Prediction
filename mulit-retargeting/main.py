...
if __name__ == '__main__':
    ...
    model = model.YumiNet(cfg.HYPER.PERIOD).to(device)
    ...
    for epoch in range(cfg.HYPER.EPOCHS):
        ...
        # Start training
        train_loss = train_epoch(model, ee_criterion, vec_criterion, col_criterion, lim_criterion, ori_criterion, fin_criterion, reg_criterion, smo_criterion,
                                 optimizer, train_loader, train_target, epoch, seq_length, logger, cfg.OTHERS.LOG_INTERVAL, writer, device)
        train_loss_all.append(train_loss)

        # Start testing
        test_loss = test_epoch(model, ee_criterion, vec_criterion, col_criterion, lim_criterion, ori_criterion, fin_criterion, reg_criterion, smo_criterion,
                               test_loader, test_target, epoch, seq_length, logger, cfg.OTHERS.LOG_INTERVAL, writer, device)
        ...
        if epoch == cfg.HYPER.EPOCHS-1:
            torch.save(model.state_dict(), os.path.join(cfg.OTHERS.SAVE, "last_model_epoch_{:04d}.pth".format(epoch)))
            logger.info("Epoch {} Model Saved".format(epoch+1).center(60, '-'))   
