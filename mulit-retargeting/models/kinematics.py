...
class ForwardKinematicsURDF(nn.Module):
    ...
    def forward(self, x, parent, offset, num_graphs, axis='z', order='xyz'):
        """
        x -- joint angles [num_graphs*num_t*num_nodes, 1]
        parent -- node parent [num_graphs*num_t*num_nodes]
        offset -- node origin(xyz-rpy) [num_graphs*num_t*num_nodes, 6]
        num_graphs -- number of graphs
        num_t -- length of sequence
        axis -- rotation axis for rotation x
        order -- rotation order for init rotation
        """
        # print('FK') 
        x = x.view(num_graphs, -1)                                                 # [20, 14] 
        # print(x.shape) 
        parent = parent.view(num_graphs, -1)[0]                                    # [14] 
        # print('parent', parent)
        offset = offset.view(num_graphs, -1, 6)                                    # [20, 14, 6]
        # print(offset.shape)
        xyz = offset[:, :, :3]                                                     # [20, 14, 3]
        # print('xyz', xyz)
        rpy = offset[:, :, 3:]                                                     # [20, 14, 3]
        # print('rpy', rpy)
...
class ForwardKinematicsAxis(nn.Module):
    ...
    def forward(self, x, parent, offset, num_graphs, axis, order='xyz'):
        ...
        """
        # print('FK')
        x = x.view(num_graphs, -1)               # [40, 18] = [20, 18]+[20, 18]
        # print(x.shape)
        parent = parent.view(num_graphs, -1)[0]  # [18] 
        # print(parent.shape)
        axis = axis.view(num_graphs, -1, 3)[0]   # [18, 3] 
        # print(axis.shape)
        axis_norm = torch.norm(axis, dim=-1)
        x = x * axis_norm                        # [40, 18]
        # print(x.shape)                        
        offset = offset.view(num_graphs, -1, 6)  # [40, 18, 6]
        # print(offset.shape)
