...
Calculate All Loss
"""
def calculate_all_loss(data_list, target_list, ee_criterion, vec_criterion, col_criterion, lim_criterion, ori_criterion, fin_criterion, reg_criterion, smo_criterion,
    ...
    if col_criterion:
        j = sum([data.t if hasattr(data, 't') else 1 for data in target_list])
        # print('target_global_pos', target_global_pos.view(j, -1, 3).shape)          [bat*t, 14, 3]
        # print('edge_index', target_list[0].edge_index_col.shape)                    [2, 12]
        # print('target_rot', target_rot.view(j, -1, 9).shape)                        [bat*t, 14, 9]
        # print('target_list', target_list[0].ee_mask_col.shape)                      [14, 1]
        col_loss = col_criterion(target_global_pos.view(j, -1, 3), target_list[0].edge_index_col,           #(bat*t, 14, 3)
                                 target_rot.view(j, -1, 9), target_list[0].ee_mask_col)*1000
    ...
    if reg_criterion:
        j = sum([data.t if hasattr(data, 't') else 1 for data in target_list])
        reg_loss = reg_criterion(z.view(j, -1, 64))
    ...
    if smo_criterion:
        smo_loss = smo_criterion(target_ang, seq_length)*5
    ...
    # total loss
    loss = ee_loss + vec_loss + col_loss + lim_loss + ori_loss + fin_loss + reg_loss + smo_loss
...
def calculate_ee_loss(data_list, target_list, target_pos, ee_criterion):
    target_mask = torch.cat([data.ee_mask for data in target_list]).to(target_pos.device)         #[280,1]
    # print(target_mask.shape)
    source_mask = torch.cat([data.ee_mask for data in data_list]).to(target_pos.device)           #[120,1]
    # print(source_mask.shape)
    target_ee = torch.masked_select(target_pos, target_mask).view(-1, 3)                          #[40,3]
    # print(target_ee.shape)
    source_ee = torch.masked_select(torch.cat([data.pos for data in data_list]).to(target_pos.device), source_mask).view(-1, 3) #[40,3]
...
class SmoothnessLoss(nn.Module):
    ...
    def forward(self, ang, seq_length):
        """
        Keyword auguments:
        ang -- joint angles [bat*t*num_nodes, 1] ---> [bat, t, num_nodes]
        """
        # calculate difference between frames
        q = ang.view(-1, 14).view(-1, seq_length, 14)
        diff1 = q[:, 1:, :] - q[:, :-1, :]    #[bat, t-1, n]
        diff2 = diff1[:, 1:, :] - diff1[:, :-1, :]   #[bat, t-2, n]
        loss = torch.mean(torch.abs(diff2))

        # calculate mask with smoothness
        # mask = torch.abs(diff) > self.smoothness
        # calculate final loss
        # loss = torch.sum(torch.exp(torch.masked_select(torch.abs(diff), mask) - self.smoothness))/q.shape[1]
