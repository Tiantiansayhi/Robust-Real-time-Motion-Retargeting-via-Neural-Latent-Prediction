...
"""
def yumi2graph(urdf_file, cfg, sliding_window):
    # load URDF
...
"""
def hand2graph(urdf_file, cfg, sliding_window):
    # load URDF
    robot = URDF.load(urdf_file)
    ...
    data.hand_temporal_edge_index = data.temporal_edge_index
...
#         # add edge attr
#         edge_attr.append(torch.Tensor(joints[child]['origin']))
#     edge_index = torch.cat([torch.stack(edge_index, dim=0) + num_nodes*i for i in range(sliding_window)], dim=0)
#     edge_index = edge_index.permute(1, 0)                       #[2, (14-2)*t]

#     edge_attr = torch.stack(edge_attr, dim=0)              
#     edge_attr = edge_attr.repeat(sliding_window, 1)             #[(14-2)*t, 6]
#     # print(edge_index, '\n', edge_attr, '\n', edge_index.shape, '\n', edge_attr.shape)
...
#     # node parent
#     # parent = -torch.ones(num_nodes*sliding_window).long()
#     parent = -torch.ones(num_nodes).long()
#     for edge in edge_index.permute(1, 0):
#         parent[edge[1]] = edge[0]
#     parent = parent.repeat(sliding_window, 1)    
#     # print('parent', parent, '\n', parent.shape)           #[14*t]

#     # node offset
#     offset = torch.stack([torch.Tensor(joints[joint]['origin']) for joint in joints_name], dim=0)
#     # offset = offset.repeat(sliding_window, 1)               #[14, 6]
#     # print('offset', offset, '\n', offset.shape)   
...
#     # move relative to origin
#     for root in cfg['root_name']:
#         # print(joints_index[root])
#         offset[joints_index[root]] -= origin
#     offset = offset.repeat(sliding_window, 1)                   #[14*6, 6]
#     # print('offset', offset, '\n', offset.shape)
...
#         dist = 0
#         current_idx = node_idx
#         while current_idx != -1 and current_idx%num_nodes != 1 and current_idx%num_nodes != 8:
#             origin = offset[current_idx]
#             offsets_mod = math.sqrt(origin[0]**2+origin[1]**2+origin[2]**2)
#             dist += offsets_mod
#             current_idx = parent[current_idx]
#         shoulder_dist[node_idx] = dist
#     # print('shoulder_dist', shoulder_dist, '\n', shoulder_dist.shape)   #[14*t,1]
...
#         dist = 0
#         current_idx = node_idx
#         while current_idx != -1 and current_idx%num_nodes != 3 and current_idx%num_nodes != 10:
#             origin = offset[current_idx]
#             offsets_mod = math.sqrt(origin[0]**2+origin[1]**2+origin[2]**2)
#             dist += offsets_mod
#             current_idx = parent[current_idx]
#         elbow_dist[node_idx] = dist
#     # print('elbow_dist', elbow_dist, '\n', elbow_dist.shape)            #[14*t,1]

#     # rotation axis
#     axis = [torch.Tensor(joints[joint]['axis']) for joint in joints_name]
#     axis = torch.stack(axis, dim=0)
#     axis = axis.repeat(sliding_window, 1)

#     # joint limit
#     lower = [torch.Tensor([joints[joint]['lower']]) for joint in joints_name]
#     lower = torch.stack(lower, dim=0)
#     lower = lower.repeat(sliding_window, 1)

#     upper = [torch.Tensor([joints[joint]['upper']]) for joint in joints_name]
#     upper = torch.stack(upper, dim=0)
#     upper = upper.repeat(sliding_window, 1)      #[14*t,1]
#     # print('lower, upper', lower.shape, upper.shape)
...
#     # skeleton
#     data = Data(x=torch.zeros(num_nodes*sliding_window, 1),
#                 edge_index=edge_index,
#                 edge_attr=edge_attr,
#                 temporal_edge_index=temporal_edge_index,
#                 skeleton_type=skeleton_type,
...
#         edge_attr.append(torch.Tensor(joints[child]['origin']))
#     edge_index = torch.cat([torch.stack(edge_index, dim=0) + num_nodes*i for i in range(sliding_window)], dim=0)
#     edge_index = edge_index.permute(1, 0)                       #[2, 17*t]

#     edge_attr = torch.stack(edge_attr, dim=0)
#     edge_attr = edge_attr.repeat(sliding_window, 1)             #[17*t, 6]
#     # print(edge_index, edge_attr, edge_index.shape, edge_attr.shape)
...
#         el_mask[joints_index[el]] = True
#     el_mask = el_mask.repeat(sliding_window, 1)
#     # print('el', el_mask.shape)

#     # node parent
#     parent = -torch.ones(num_nodes*sliding_window).long()          #[18*t]
#     for edge in edge_index.permute(1, 0):
#         parent[edge[1]] = edge[0]
#     # print('parent', parent, '\n', parent.shape)
...
#         offset.append(torch.Tensor(joints[joint]['origin']))
#     offset = torch.stack(offset, dim=0)
#     offset = offset.repeat(sliding_window, 1)                      #[18*t, 6]
#     # print('offset', offset, '\n', offset.shape)
...
#         dist = 0
#         current_idx = node_idx
#         while current_idx != -1 and current_idx%num_nodes != 1 and current_idx%num_nodes != 4 and current_idx%num_nodes != 7 and current_idx%num_nodes != 10 and current_idx%num_nodes != 13:
#             origin = offset[current_idx]
#             offsets_mod = math.sqrt(origin[0]**2+origin[1]**2+origin[2]**2)
#             dist += offsets_mod
#             current_idx = parent[current_idx]
...
#     axis = torch.stack(axis, dim=0)
#     axis = axis.repeat(sliding_window, 1)
#     # print('axis', axis, axis.shape)                                         #[18, 3]

#     # joint limit
#     lower = [torch.Tensor([joints[joint]['lower']]) if joint != cfg['root_name'] else torch.zeros(1) for joint in joints_name]
#     lower = torch.stack(lower, dim=0)
#     lower = lower.repeat(sliding_window, 1)                                   #[18*t, 1]

#     upper = [torch.Tensor([joints[joint]['upper']]) if joint != cfg['root_name'] else torch.zeros(1) for joint in joints_name]
#     upper = torch.stack(upper, dim=0)
#     upper = upper.repeat(sliding_window, 1)                                   #[18*t, 1]
#     # print('lower', lower, lower.shape, '\n', 'upper', upper, upper.shape)
...
# def yumi_dist():    
    ...
    graph = yumi2graph(urdf_file='/home/wtt/Data/multi-frame/sign-all/target/yumi-all/yumi_with_hands.urdf', cfg=yumi_cfg, sliding_window=5)
    # #return graph.root_dist.numpy(), graph.shoulder_dist.numpy(), graph.elbow_dist.numpy()
...
# def hand_dist(): 
    ...
    # graph = hand2graph(urdf_file='/home/wtt/Data/multi-frame/sign-all/target/yumi-all/yumi_with_hands.urdf', cfg=hand_cfg, sliding_window=5)
